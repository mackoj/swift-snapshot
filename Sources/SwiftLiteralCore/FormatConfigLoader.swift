...
enum FormatConfigLoader {
  ...
  /// - Throws: `LiteralError.formatting` if the file exists but cannot be parsed
  static func loadProfile(from source: FormatConfigSource?) throws -> FormatProfile {
    guard let source = source else {
      return LiteralConfig.libraryDefaultFormatProfile()
    }
  ...
  private static func loadFromSwiftFormat(_ url: URL) throws -> FormatProfile {
    ...
    guard let json = try JSONSerialization.jsonObject(with: data) as? [String: Any] else {
      throw LiteralError.formatting("Invalid .swift-format file format")
    }
  ...
  ///   - file: Name of the configuration file to find
