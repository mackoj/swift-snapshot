...
SwiftLiteral integrates with standard formatting tools to ensure generated code matches your project's style:
...
All formatting options are configured via ``LiteralConfig`` and processed by ``CodeFormatter``.

## Configuration Sources

Specify the format configuration source using ``LiteralConfig/setFormatConfigSource(_:)``:

```swift
// Use .editorconfig file
let editorconfigURL = URL(fileURLWithPath: "/path/to/project/.editorconfig")
LiteralConfig.setFormatConfigSource(.editorconfig(editorconfigURL))

// Use .swift-format file
let swiftFormatURL = URL(fileURLWithPath: "/path/to/project/.swift-format")
LiteralConfig.setFormatConfigSource(.swiftFormat(swiftFormatURL))
```

## .editorconfig Format

SwiftLiteral supports the following `.editorconfig` properties:
...
If no configuration is specified, SwiftLiteral uses these defaults:
