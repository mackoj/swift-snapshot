...
Understanding SwiftLiteral's design, components, and implementation.

## Overview

SwiftLiteral is built on a layered architecture that separates concerns and provides clear extension points. The library follows Swift best practices and integrates with industry-standard tools.
...
The ``Literal`` enum provides the main entry point:
...
1. **Custom Renderer Check**: Queries ``ValueRendererRegistry``
...
``LiteralConfig`` provides thread-safe global configuration:
...
``ValueRendererRegistry`` enables type-specific rendering:
...
ValueRenderer.render(value, context)
   ↓
1. Check ValueRendererRegistry for custom renderer
...
2. Global header via `LiteralConfig.setGlobalHeader()`
...
``LiteralError`` provides detailed error information:
...
- ``LiteralConfig`` - Configuration
- ``ValueRendererRegistry`` - Custom renderers
