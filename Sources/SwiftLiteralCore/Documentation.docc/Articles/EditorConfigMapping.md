...
This document describes how EditorConfig properties are mapped to swift-format configuration and post-processing in SwiftLiteral.

## Overview

SwiftLiteral uses a two-stage approach to apply EditorConfig settings:
...
SwiftLiteral resolves EditorConfig properties for Swift files by processing sections in order:
...
If no EditorConfig file is specified, SwiftLiteral uses these defaults:
...
- `charset` - SwiftLiteral always uses UTF-8
