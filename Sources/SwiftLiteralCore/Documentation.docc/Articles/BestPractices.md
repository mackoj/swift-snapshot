...
Proven patterns and guidelines for using SwiftLiteral effectively in your projects.

## Overview

This guide covers best practices for organizing, naming, and using SwiftLiteral fixtures to maintain clean, maintainable test and development workflows.
...
```swift
// ✅ Good: Clear intent
try user.writeLiteral(variableName: "activeAdminUser")
try user.writeLiteral(variableName: "suspendedUserWithPendingOrders")

// ❌ Avoid: Generic or unclear names
try user.writeLiteral(variableName: "user1")
try user.writeLiteral(variableName: "testData")
```
...
```swift
@SwiftLiteral(folder: "Fixtures/User")
struct User {
    let id: String
    let name: String
}
...
Use SwiftLiteral to capture complex test data and create reusable fixtures that improve test reliability and maintainability.
...
func testOrderProcessing() throws {
    ...
    try order.writeLiteral(testName: #function)
    
    // Run your test logic
    let result = orderService.process(order)
    
    // Export result for verification
    try result.writeLiteral(variableName: "processedOrder")
}
```
...
func testJiraTicket_ABC123_UserProfileCrash() throws {
    ...
    try problematicUser.writeLiteral(testName: #function)
...
```swift
#if DEBUG
try user.writeLiteral(testName: #function)
#endif
```

SwiftLiteral already uses `#if DEBUG` internally, so export calls in production builds become no-ops with minimal overhead.
...
try sanitizedUser.writeLiteral(
...
try user.writeLiteral(variableName: "minimalUser")
...
- <doc:BasicUsage> - Getting started with SwiftLiteral
- <doc:CustomRenderers> - Handling special types
- ``Literal`` - Runtime API reference
- ``SwiftLiteral`` - Macro API reference
