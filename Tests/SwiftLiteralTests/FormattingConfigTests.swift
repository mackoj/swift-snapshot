...
extension LiteralTests {
  ...
  @Suite struct FormattingConfigTests {
    ...
    let source = LiteralConfig.getFormatConfigSource()
    #expect(source != nil)
    ...
    // Set the config source
    LiteralConfig.setFormatConfigSource(.swiftFormat(configURL))
    ...
    @Test func snapshotExportWithMissingConfigFile() throws {
      ...
      LiteralConfig.setFormatConfigSource(.swiftFormat(configURL))
      defer { LiteralConfig.resetToLibraryDefaults() }
